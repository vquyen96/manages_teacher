...
$query = "SELECT * FROM don_vi_cong_tac WHERE trang_thai = 1 ORDER BY id DESC
...
if (isset($_GET['action']) && $_GET['action'] == 'sua') {
    ...
    try {
        ...
        $queryEdit = "SELECT * FROM don_vi_cong_tac WHERE id = ? LIMIT 0,1";
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <?php if ($num > 0) { ?>
                                        <table id="example1" class="table table-bordered table-striped">
                                            ...
                                            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="don_vi.php<?php echo '?id='.$id.'&action=sua'?>" class="btn btn-primary">Sửa</a>
                                                        <a href="xoa_don_vi.php<?php echo '?id='.$id.'&action=xoa'?>" class="btn btn-danger" onclick="return confirm('Bạn chắc chắn muốn XÓA')">Xóa</a>
                                                    </td>
                                                </tr>
                                            <?php }?>
                                        </table>
                                        ...
                                        echo "<div class='alert alert-danger'>Không tìm thấy đơn vị công tác nào.</div>";
                                    }
                                    ?>
                                </div>
                                <div class="col-sm-6">
...
if($_POST){
    ...
    if ($method == 'post') {
        try{
            // truy vấn INSERT
            $query = "INSERT INTO don_vi_cong_tac SET ten=:ten, chi_tiet=:chi_tiet, trang_thai=:trang_thai, ngay_tao=:ngay_tao";
            ...
            // Thực thi truy vấn
            if($stmt->execute()){
                //bị lỗi không chuyển trang bằng php nên phải dùng javascript
                echo '<script type="text/javascript">location.href = "don_vi.php";</script>';
                if (headers_sent()) {
    ...
    if ($method == 'put') {
        try{
            // truy vấn INSERT
            $query = "UPDATE don_vi_cong_tac SET ten=:ten, chi_tiet=:chi_tiet, trang_thai=:trang_thai, ngay_cap_nhat=:ngay_cap_nhat WHERE id=:id";
